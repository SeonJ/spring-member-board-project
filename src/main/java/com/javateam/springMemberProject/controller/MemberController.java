...
//	public String viewAction(HttpServletRequest request) {
	...
	public String updateAction(MemberUpdateVO memberUpdateVO, 
		...
		log.info("기존의 회원 정보 : " + (MemberVO)session.getAttribute("SESS_DEFAULT_MEMBER"));
		...
		// MemberVO loginInfo = (MemberVO)session.getAttribute("SESS_LOGIN_MEMBER");
		MemberVO loginInfo = member;
		
		if (msg.contentEquals("회원 정보 수정에 성공하였습니다.")) {
			// 사용자 : 관리자(1) => 전체회원 조회 페이지
			// 사용자 : 일반회원(0) => MyPage
	...
	public String deleteMember(@RequestParam("userid") String userid,
		...
		if (flag == true) { // 삭제 성공
			...
			if (admin == 1) { // 관리자
				...
				movePageError = "member/my_page.do";
			}
